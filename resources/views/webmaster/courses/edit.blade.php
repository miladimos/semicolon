...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active"><a href="#">ویرایش دوره</a>
    </li>
@endsection
                ...
                <div class="card-body">
                    <ul class="nav nav-tabs mb-3" role="tablist">
                        <li class="nav-item">
                            <a class="nav-link d-flex align-items-center active" id="account-tab" data-toggle="tab"
                                ...
                                <i class="feather icon-user mr-25"></i><span class="d-none d-sm-block">ویرایش دوره</span>
                            </a>
                        </li>
                    </ul>
                    <div class="tab-content">
                        <div class="tab-pane active" id="account" aria-labelledby="account-tab" role="tabpanel">
                            ...
                            <form action="{{ route('webmaster.courses.update', $course->uuid) }}" method="POST"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" name="type_id">
                                                @foreach ($types as $type)
                                                    <option value="{{ $type->id }}" @if ($type->id === $course->type->id) selected @endif>{{ $type->title }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" name="level_id">
                                                @foreach ($levels as $level)
                                                    <option value="{{ $level->id }}" @if ($level->id === $course->level->id) selected @endif>{{ $level->title }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" name="status_id">
                                                @foreach ($statuses as $status)
                                                    <option value="{{ $status->id }}" @if ($status->id === $course->status->id) selected @endif>{{ $status->title }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                    ...
                                    <div class="col-12 col-sm-6">
                                        <div class="form-group">
                                            <div class="custom-control custom-checkbox">
                                                <input type="checkbox" name="imteacher" id="imteacher"
                                                    class="custom-control-input" @if (auth()->id() === $course->teacher->id) checked @endif>
                                            ...
                                            <select class="form-control select2" name="teacher_id">
                                                ...
                                                @foreach ($users as $teacher)
                                                    <option value="{{ $teacher->id }}" @if ($teacher->id === $course->teacher->id) selected @endif>{{ $teacher->label }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <div class="custom-control custom-checkbox">
                                                <input type="checkbox" name="imauthor" id="imauthor"
                                                    class="custom-control-input" @if (auth()->id() === $course->author->id) checked @endif>
                                            ...
                                            <select class="form-control select2" name="author_id">
                                                ...
                                                @foreach ($users as $author)
                                                    <option value="{{ $author->id }}" @if ($author->id === $course->author->id) selected @endif>{{ $author->label }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" name="categories[]" multiple="multiple">
                                                @foreach ($categories as $category)
                                                    <option value="{{ $category->id }}" @if (in_array($category->id, $course->categories->pluck('id')->toArray())) selected @endif>{{ $category->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" name="tags[]" multiple="multiple">
                                                @foreach ($tags as $tag)
                                                    <option value="{{ $tag->id }}" @if (in_array($tag->id, $course->tags->pluck('id')->toArray())) selected @endif>{{ $tag->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <div class="custom-control custom-checkbox">
                                                <input type="checkbox" name="active" id="active"
                                                    class="custom-control-input" @if ($course->active) checked @endif>
                                        ...
                                        <div class="form-group">
                                            <div class="controls">
                                                ...
                                                <textarea class="form-control" name="description" id="description" required
                                                    ...
                                                    rows="8">{{ old('description') ?? $course->description }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-12 col-md-12 col-lg-12">
                                        <div class="form-group">
                                            <div class="controls">
                                                ...
                                                <textarea class="form-control" name="body" id="body" required
                                                    ...
                                                    rows="8">{{ old('body') ?? $course->body }}</textarea>
                                            </div>
                                        </div>
                                    </div>
